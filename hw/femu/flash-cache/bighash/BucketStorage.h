...
class FOLLY_PACK_ATTR BucketStorage {
 public:
  ...
  // traverse and print
  void traverse_print() {
    std::cout << "capacity: " << capacity_ << " numAllocations: " << numAllocations_ << std::endl;
    Allocation alloc = getFirst();
    uint32_t i = 0;
    while (!alloc.done()) {
      alloc = getNext(alloc);
      i++;
    }
    if(i != numAllocations_)
        std::cout << "traverse_print error: " << i << " " << numAllocations_ << std::endl;
  }
